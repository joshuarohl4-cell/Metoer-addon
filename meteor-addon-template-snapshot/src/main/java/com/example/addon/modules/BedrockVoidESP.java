...
public class BedrockVoidESP extends Module {
    ...
    public BedrockVoidESP() {
        super(AddonTemplate.CATEGORY, "bedrock-void-esp", "Finds voids in bedrock layers. Useful for base finding on Donut SMP.");
    }
                ...
                for (int dz = 0; dz < 16; dz++) {
                    ...
                    if (group.size() >= minVoidSize.get() && isVoidEnclosed(group)) {
                        ...
                        if (!group.isEmpty()) {
                            ...
                            sendVoidMessage("§5[§dVoidESP§5] §bVoid found§5: §b" + group.size() + " blocks at " + firstBlock.toShortString());
                        }
                    }
                }
            }
